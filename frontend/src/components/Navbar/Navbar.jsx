        ...
        {user ? (
          <div className={`header-inner ${role === "admin" ? "admin" : ""}`}>
            <nav className="nav-left">
              ...
              {role === "admin" ? (
                <>
                  <Link to="#" className="nav-icon"><FaTools /></Link>
                  <Link to={`/profile/${userId}`} className="nav-icon"><FaUser /></Link>
                  <Link to="#" className="nav-icon"><MdOutlineExplore /></Link>
                </>
              ) : (
                <>
                  <Link to="/" className="nav-icon"><IoHomeOutline /></Link>
                  <Link to="#" className="nav-icon"><MdOutlineExplore /></Link>
                  <Link to="#" className="nav-icon"><BsPencilFill /></Link>
                </>
              )}
            </nav>
                      ...
                      {searchResults.results.artworks?.length > 0 && (
                        <div className="results-section">
                          ...
                          {searchResults.results.artworks.map((a) => (
                            <div key={a._id} className="result-item" onClick={() => handleResultClick("artwork", a)}>
                              <span className="result-title">{a.title}</span>
                              {a.medium && <span className="result-subtitle">{a.medium}</span>}
                            </div>
                          ))}
                        </div>
                      )}
                      {searchResults.results.users?.length > 0 && (
                        <div className="results-section">
                          ...
                          {searchResults.results.users.map((u) => (
                            <div key={u._id} className="result-item" onClick={() => handleResultClick("user", u)}>
                              <span className="result-title">{u.firstName} {u.lastName}</span>
                      ...
                      {searchResults.results.categories?.length > 0 && (
                        <div className="results-section">
                          ...
                          {searchResults.results.categories.map((c) => (
                            <div key={c._id} className="result-item" onClick={() => handleResultClick("category", c)}>
          ...
          <div className="header-inner guest">
            <Link to="/login" className="nav-logo"><img src={logo} alt="Logo" /></Link>
